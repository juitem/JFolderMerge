...
# Parse --public flag and remove it from arguments to keep existing logic working
ARGS=()
HOST="127.0.0.1"
for arg in "$@"; do
    if [ "$arg" == "--public" ]; then
...
# Reset position arguments to the filtered list
set -- "${ARGS[@]}"
...
# usage: ./run.sh [LEFT] [RIGHT] [PORT] [--public]
if [ "$#" -eq 1 ]; then
    if [[ "$1" =~ ^[0-9]+$ ]]; then
        PORT=$1
        LEFT=$DEFAULT_LEFT
        ...
        echo "Usage: ./run.sh [PORT] OR ./run.sh [LEFT] [RIGHT] [PORT] [--public]"
        exit 1
    fi
elif [ "$#" -ge 2 ]; then
...
if [ "$HOST" == "0.0.0.0" ]; then
    if [ ! -z "$IP" ]; then
        echo "URL (External): http://$IP:$PORT"
         ...
         echo "URL (External): http://0.0.0.0:$PORT (Check your IP manually)"
    fi
else
    echo "Mode:           Localhost Only (Use --public to expose)"
fi
...
./venv/bin/python3 -m backend.main --left "$LEFT" --right "$RIGHT" --port "$PORT" --host "$HOST" &
APP_PID=$!
wait $APP_PID
